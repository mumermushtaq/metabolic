...
Locator: locator('table tr').nth(1)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('table tr').nth(1)
...
> 32  |     await expect(page.locator('table tr').nth(1)).toBeVisible();
      |                                                   ^ Error: expect(locator).toBeVisible() failed
  33  |   });
  34  | 
  ...
  77  |     await expect(page.locator('table tr').nth(1)).toBeVisible();
  78  |   });
  79  | 
  ...
  96  |     await expect(page.locator('table tr')).toHaveCount(1); // only header row remains
  97  |   });
  98  | 

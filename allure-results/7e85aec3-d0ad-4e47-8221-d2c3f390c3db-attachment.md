...
- Name: e2e\patient-listings.spec.js >> Patient Listings — @patients >> should open add patient form when clicking Add Patient
- Location: tests\e2e\patient-listings.spec.js:244:3
...
> 60  |     await this.page.goto(`${process.env.BASE_URL}/patient`);
  ...
  125 |     // Click the dropdown indicator arrow (not the whole control — avoids toggle-close)
  126 |     // Options render as .filterSelect__option in a react-select portal
  127 |     await this.page.locator('.filterSelect__indicators').first().click();
  128 |     await this.page.locator('.filterSelect__option')
  129 |       .filter({ hasText: gender }).first().click();
  130 |   }
  131 | 
  132 |   async saveFilterSet(name) {
  133 |     await this.saveFilterSetBtn.click();
  134 |     const titleInput = this.page.getByRole('textbox', { name: 'Filter Title' });
  135 |     await titleInput.waitFor({ state: 'visible' });
  136 |     await titleInput.fill(name);
  137 |     // Second radio option sets filter visibility scope (matches codegen)
  138 |     await this.page.locator('.Filter_radio__3CaS3 > div:nth-child(2) > label').click();
  139 |     await this.page.getByRole('button', { name: 'Save Filter' }).click();
  140 |     await this.page.waitForTimeout(process.env.CI ? 2000 : 800);
  141 |   }
  142 | 
  143 |   // ── Saved Filters ────────────────────────────────────────────
  144 | 
  145 |   async clickSavedFilter(filterName) {
  146 |     await this.page.getByText(filterName, { exact: false }).first().click();
  147 |     await this.page.waitForTimeout(process.env.CI ? 2000 : 800);
  148 |   }
  149 | 
  150 |   // ── Table interactions ───────────────────────────────────────
  151 | 
  152 |   async expandPatientRow(index = 0) {
  153 |     const rows = this.page.locator('tr').filter({ has: this.page.locator('td') });
  154 |     await rows.nth(index).locator('[aria-label="expand"], svg, button').first().click();
  155 |   }
  156 | 
  157 |   async openPatient(patientName) {
  158 |     await this.page.getByText(patientName).first().click();

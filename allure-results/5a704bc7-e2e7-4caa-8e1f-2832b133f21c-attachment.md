...
- Name: e2e\glucose-analysis.spec.js >> Glucose Analysis >> changes date range and applies
- Location: tests\e2e\glucose-analysis.spec.js:24:3
...
```ts
  ...
  5  | 
  6  |     // ── Navigation ────────────────────────────────────────────────
  7  |     this.glucoseAnalysisTab = page.getByRole('link', { name: 'Glucose Analysis' });
  8  | 
  9  |     // ── Date range picker ─────────────────────────────────────────
  10 |     // Start date button shows current start date e.g. "Jun 19 "
  11 |     this.startDateBtn = page.getByRole('button', { name: /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d+/ }).first();
  12 |     this.applyBtn     = page.getByRole('button', { name: 'Apply' });
  13 |   }
  14 | 
  15 |   // ── Navigate directly to Glucose Analysis tab ─────────────────
  16 |   async openGlucoseAnalysisTab(patientId) {
  17 |     await this.page.goto(`${process.env.BASE_URL}/patient/profile/glucose-analysis/${patientId}`);
  18 |     await this.glucoseAnalysisTab.waitFor({ state: 'visible', timeout: 15000 });
  19 |   }
  20 | 
  21 |   // ── Open the date range picker ────────────────────────────────
  22 |   async openDatePicker() {
> 23 |     await this.startDateBtn.waitFor({ state: 'visible', timeout: 10000 });
     |                             ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  24 |     await this.startDateBtn.click();
  25 |   }
  26 | 
  27 |   // ── Select a specific day from the open calendar ──────────────
  28 |   // dayNumber: e.g. 14
  29 |   async selectDay(dayNumber) {
  30 |     await this.page
  31 |       .locator('div')
  32 |       .filter({ hasText: new RegExp(`^${dayNumber}$`) })
  33 |       .click();
  34 |   }
  35 | 
  36 |   // ── Apply the selected date range ────────────────────────────
  37 |   async applyDateRange() {
  38 |     await this.applyBtn.click();
  39 |     // Wait for chart/data to reload after applying
  40 |     await this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  41 |   }
  42 | 
  43 |   // ── Full flow: open picker, select day, apply ─────────────────
  44 |   async changeDateRange(dayNumber) {
  45 |     await this.openDatePicker();
  46 |     await this.selectDay(dayNumber);
  47 |     await this.applyDateRange();
  48 |   }
  49 | }
  50 | 
  51 | module.exports = { GlucoseAnalysisPage };
```
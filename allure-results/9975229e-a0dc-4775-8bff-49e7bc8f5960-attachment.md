...
- Name: e2e\documents.spec.js >> Documents >> deletes the uploaded document
- Location: tests\e2e\documents.spec.js:71:3
...
Call log:
  - waiting for locator('div:nth-child(3) > [class*="medicalCardsWrap"] > [class*="medicalCardsSingle"] > [class*="theme_border"] > [class*="medicalReportDelete"] > svg')
...
> 131 |         .click();
  ...
  208 |     if (count === 0) {
  209 |       throw new Error('No documents available to delete');
  210 |     }
  211 |     // Use last() if index exceeds available count
  212 |     const safeIndex = Math.min(index, count - 1);
  213 |     await deleteIcons.nth(safeIndex).click();
  214 |     await this.deleteConfirmBtn.waitFor({ state: 'visible', timeout: 5000 });
  215 |     await this.deleteConfirmBtn.click();
  216 |     await this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  217 |   }
  218 | }
  219 | 
  220 | module.exports = { DocumentsPage };
```
...
- Name: e2e\visits.spec.js >> Visits >> opens a past visit via direct URL
- Location: tests\e2e\visits.spec.js:59:3
...
Call log:
  - waiting for getByRole('button', { name: 'Begin Consultation' }) to be visible
      ...
      - generic [ref=e85]:
        ...
        - generic [ref=e113]:
          - region "Current Visits (40)" [ref=e114]:
            ...
            - generic [ref=e116]:
              - button "Metabolic Baseline Protocol- 40 to 50 371050 Jumeirah First Active June 26, 2026 4:07 PM 3 days ago Start Consultation" [active] [ref=e117] [cursor=pointer]:
                - generic [ref=e118]:
                  - generic [ref=e119]:
                    - generic [ref=e120]:
                      - generic [ref=e121]:
                  ...
                  - generic [ref=e128]:
                    ...
                    - generic [ref=e139]: 3 days ago
                  - link "Start Consultation" [ref=e140]:
...
```ts
  ...
  5  | 
  6  |     // ── Navigation ────────────────────────────────────────────────
  7  |     this.visitsTab = page.getByRole('link', { name: 'Visits' });
  8  |   }
  9  | 
  10 |   // ── Navigate directly to Visits tab ──────────────────────────
  11 |   async openVisitsTab(patientId) {
  12 |     await this.page.goto(`${process.env.BASE_URL}/patient/profile/visits/${patientId}`);
  13 |     await this.visitsTab.waitFor({ state: 'visible', timeout: 15000 });
  14 |     await this.page.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  15 |   }
  16 | 
  17 |   // ── Get count of visit cards visible on the page ─────────────
  18 |   async getVisitCount() {
  19 |     const cards = this.page.getByRole('button').filter({ hasText: /Metabolic|Protocol|Active|Pending/i });
  20 |     return cards.count();
  21 |   }
  22 | 
  23 |   // ── Open a visit by index — returns the popup page ────────────
  24 |   // Visits open in a new tab (popup)
  25 |   async openVisitByIndex(index = 0) {
  26 |     const cards = this.page.getByRole('button').filter({ hasText: /Metabolic|Protocol|Active|Pending/i });
  27 |     const popupPromise = this.page.waitForEvent('popup');
  28 |     await cards.nth(index).click();
  29 |     const popup = await popupPromise;
  30 |     await popup.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
  31 |     return popup;
  32 |   }
  33 | 
  34 |   // ── Open a visit from the past visits section ─────────────────
  35 |   async openPastVisitByIndex(index = 0) {
  36 |     const pastSection = this.page.getByTestId('past-visits-scroll');
  37 |     const cards = pastSection.getByRole('button').filter({ hasText: /Metabolic|Protocol/i });
  38 |     const popupPromise = this.page.waitForEvent('popup');
  39 |     await cards.nth(index).click();
  40 |     const popup = await popupPromise;
  41 |     await popup.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
  42 |     return popup;
  43 |   }
  44 | 
  45 |   // ── Begin consultation on a visit popup ───────────────────────
  46 |   async beginConsultation(visitPopup) {
  47 |     const beginBtn = visitPopup.getByRole('button', { name: 'Begin Consultation' });
> 48 |     await beginBtn.waitFor({ state: 'visible', timeout: 10000 });
     |                    ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  49 |     await beginBtn.click();
  50 |   }
  51 | 
  52 |   // ── Start review on a visit popup ────────────────────────────
  53 |   async startReview(visitPopup) {
  54 |     const startBtn = visitPopup.getByRole('button', { name: 'Start Review' });
  55 |     await startBtn.waitFor({ state: 'visible', timeout: 10000 });
  56 |     await startBtn.click();
  57 |   }
  58 | 
  59 |   // ── Click Next through all steps of the consultation ─────────
  60 |   async clickNextUntilEnd(visitPopup, maxSteps = 15) {
  61 |     let steps = 0;
  62 |     while (steps < maxSteps) {
  63 |       const nextBtn = visitPopup.getByRole('button', { name: 'Next' });
  64 |       const visible = await nextBtn.isVisible({ timeout: 3000 }).catch(() => false);
  65 |       if (!visible) break;
  66 |       await nextBtn.click();
  67 |       await visitPopup.waitForTimeout(300);
  68 |       steps++;
  69 |     }
  70 |     return steps;
  71 |   }
  72 | 
  73 |   // ── Navigate to a specific consultation URL directly ─────────
  74 |   async openConsultationDirect(visitPopup, patientId, visitId) {
  75 |     await visitPopup.goto(
  76 |       `${process.env.BASE_URL}/patient/profile/consultation/${patientId}?visitId=${visitId}`
  77 |     );
  78 |     await visitPopup.waitForLoadState('networkidle', { timeout: 10000 }).catch(() => {});
  79 |   }
  80 | }
  81 | 
  82 | module.exports = { VisitsPage };
```
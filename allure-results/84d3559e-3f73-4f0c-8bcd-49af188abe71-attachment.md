...
Call log:
  - waiting for getByRole('button', { name: /^(\\d{4}\\s+)?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\\s+\\d+/ }).first() to be visible
...
```ts
  ...
  7  |       name: /^(\\d{4}\\s+)?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\\s+\\d+/,
  8  |     }).first();

      ...
      - generic [ref=e391]:
        - generic [ref=e392]:
          ...
          - generic [ref=e396]:
            ...
            - textbox "Last Name" [ref=e398]: User474730
          - generic [ref=e399]:
...
```ts
  ...
  63  | 
  64  |     // Wait for the dialog (and its backdrop) to fully close before returning —
  65  |     // otherwise the backdrop intercepts subsequent clicks on the page behind it
  66  |     await dialog.waitFor({ state: 'hidden', timeout: 15000 }).catch(() => {});
  67  |     await this.page.locator('.offcanvas-backdrop').waitFor({ state: 'hidden', timeout: 10000 }).catch(() => {});
  68  |   }
  69  | 
  70  |   // ── Search for a team member ───────────────────────────────────
  71  |   async searchUser(query) {
> 72  |     await this.searchInput.click();
      |                            ^ Error: locator.click: Test timeout of 60000ms exceeded.
  73  |     await this.searchInput.fill(query);
  74  |     await this.page.waitForTimeout(500);
  75  |   }
  76  | 
  77  |   // ── Clear active filter ─────────────────────────────────────────
  78  |   async clearFilter() {
  79  |     await this.clearFilterBtn.click();
  80  |   }
  81  | 
  82  |   // ── Open row actions dropdown by row name and select an option ──
  83  |   async openRowMenuByName(rowName, optionText) {
  84  |     const row = this.page.getByRole('row', { name: new RegExp(rowName) });
  85  |     await row.locator('#dropdown-basic').click();
  86  |     await this.page.getByText(optionText, { exact: true }).click();
  87  |   }
  88  | 
  89  |   // ── Edit a user by name (search → open dropdown → Edit Info) ───
  90  |   async editUserByName(fullName, updates = {}) {
  91  |     await this.searchUser(fullName);
  92  |     await this.page.getByText(fullName, { exact: true }).click();
  93  | 
  94  |     // Open the row's dropdown menu via the cell dropdown selector
  95  |     const dropdownBtn = this.page.locator('[id^="row-"] [id^="cell-"] #dropdown-basic').first();
  96  |     await dropdownBtn.click();
  97  |     await this.editInfoOption.click();
  98  | 
  99  |     // Apply any field updates here if needed in the future
  100 |     await this.editUserBtn.click();
  101 |   }
  102 | }
  103 | 
  104 | module.exports = { TeamPage };
```